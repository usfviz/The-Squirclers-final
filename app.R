...
ui <- fluidPage(theme = shinytheme('darkly'), headerPanel("Major League Baseball Strikezone Visualization Tool"),
  ...
  tabPanel("Strike Likelihood", selectizeInput("batside", "Select Batter Side",
                                     choice = c("Right", "Left")),
           plotlyOutput("summary")),  # tabPanel
  tabPanel('Strike Zone Close-Up', 
           ...
           tabPanel('Pitch Heatmap', 
                    fluidRow(column(2, offset=0,  radioButtons(inputId = "stanceSelection4",
                                                               label = "Select Batter Stance", 
                                                               choiceNames = c("Right Hand", "Left Hand"),
                                                               choiceValues = c("R", "L"),
                                                               selected = "R"
                    ),
                    selectInput(inputId="countSelect4", 
                                label="Select the Count", 
                                choices  = c("0-0",
                                             "0-1",
                                             "0-2",
                    ...
                    column(7, offset=0, plotlyOutput("heatmap", width = 500, height = 500)),
                    column(3, offset=0, heatmapDescript)
                    )
  )
      ######################################## end of TabPanel
)
...
server <- function(input, output) {
     ...
     plotHeatMap <- reactive({
       pitch_data_trial <- pitch_data2[pitch_data2$bat_side == input$stanceSelection4 & pitch_data2$count1 == input$countSelect4,]
       
       pitch_data_use <- pitch_data_trial[c(14,15)]
       
       
       pitch_heat <- dcast( pitch_data_use , Y_Region ~X_Region , length )
       pitch_heat_melt <- melt(pitch_heat, id.vars = c('Y_Region'))
       pitch_heat_melt$Y_Region <- factor(pitch_heat_melt$Y_Region)
       colnames(pitch_heat_melt) <- c('Y_Region', 'X_Region', 'Count')
       hm.palette <- colorRampPalette(rev(brewer.pal(11, 'Spectral')), space='Lab')  
       
       
       p <- ggplot(data = pitch_heat_melt, aes(x = X_Region, y = Y_Region)) 
